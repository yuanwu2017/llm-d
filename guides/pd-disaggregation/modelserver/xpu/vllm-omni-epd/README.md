# Wan Text-to-Video on Intel XPU — Disaggregated Encode/Decode (EPD, two-pool)
...
- **decode pool** — runs the DiT denoise loop + VAE decode (Stage 1, `model_stage=dit`),
...
[`../vllm-omni/`](../vllm-omni/). The two pools scale independently — encode is a
light UMT5 pass, decode is the heavy, throughput-dominant DiT loop.
...
runtime and hold ~13.5 GiB of weights. Splitting them lets you scale the decode
...
| File | Purpose |
|---|---|
| `stage-config.configmap.yaml` | The validated 2-stage topology `wan2_2_epd.yaml`, mounted at `/etc/wan/` in both pods. |
| `encode-deployment.yaml` | Encode pod (master + API + Stage 0) **and** the **`omni-master` Service** (required). |
| `decode-deployment.yaml` | Headless decode pod (Stage 1), connects to the master via the `omni-master` Service. |
...
| `httproute.yaml` | Path-scoped `HTTPRoute` sending `/v1/videos` to `InferencePool wan-video-epd-xpu`. |
...
> (the headless decode pod's ZMQ stage sockets connect to it). The HTTP
...
>
> **No `namePrefix`** is applied: the decode pod reaches the master via the fixed
...
```bash
cd guides/pd-disaggregation/modelserver/xpu/vllm-omni-epd
...
kubectl create namespace wan-epd
kubectl kustomize . | less
kubectl apply -k . -n wan-epd

# 3) Route /v1/videos and install the router/EPP profile.
kubectl apply -f httproute.yaml -n wan-epd
helm upgrade --install wan-video-epd-xpu <router-chart> \
  -f guides/recipes/router/base.values.yaml \
  -f router.values.yaml
```

The decode pod is co-scheduled on the same node as the master for single-node
...
```bash
kubectl port-forward deploy/encode 8000:8000 -n wan-epd
...
encode and decode stages ran on their respective pools.

## Scaling

- Scale the **decode** Deployment `replicas` independently (it is the throughput
  bottleneck). The EPP load-balances across master replicas; decode replicas
  register to the master and are picked by the master's StagePool
  (`--omni-lb-policy`).
- For Wan2.2 A14B MoE, raise the decode claim `count` and set
...
containers (encode on card 0, decode on card 1): `POST /v1/videos/sync` returned
